...
<script type="text/javascript">
	...
	<!-- 페이징 -->
	<script type="text/javascript">	// news
		...
		function pageGo(){
		    ...
		    for(var i = pageStartIndex; i <= pageLastIndex; i++){
				...
				$('#article').append('<p>'+news.items[i].pubDate+'</p><br>');
		    }
		    if(pageNum==1){
	...
	<script type="text/javascript">
      var corList = ${corList };
          ...
          $(document).ready(corPageGo);
       
      var corPageNum = 1;
       var totalCors = corList.length;         // 총 기사수
       var corPerPage = 5;                     // 한페이지당 기사 수
       var totalCorPage = parseInt(totalCors/corPerPage)+1;   // 총페이지수(마지막 페이지 수)
       if(totalCors%5 == 0){
    	   totalCorPage -= 1;
       }
      
      function corPageDown(){
         if(corPageNum != 1){
            corPageNum--;
         }
         corPageGo();
      ...
      function corPageGo(){
         if(corList.length=="0"){
          ...
          var corPageStartIndex = parseInt((corPageNum-1)*5);      //  현재페이지 시작 기사 인덱스
          var corPageLastIndex = (corPageNum == totalCorPage) ? totalCors-1 : corPageNum * corPerPage -1;   // 현재페이지 마지막 기사 인덱스
          ...
          $("#corTd3").empty();
          $("#corTd4").empty();
          /* for(var i = corPageStartIndex; i <= corPageLastIndex; i++){ */
          for(var i = corPageStartIndex; i <= corPageLastIndex; i++){
            //$("#corLis".concat('t')).append('<form action="/corporation/search" method="post"><input type="hidden" name="c_no" value='+corList[i].c_no+'><input type="submit" class="btn" value='+corList[i].c_name+'>');
            // 문자열만 concat가능여
             // concat 이 안된다 ㄱㄷㄱㄷㄱㄷㄱㄷㄱㄷㄱㄷ
            $('#corTd'.concat(new String(i%5))).append('<form action="/corporation/search" method="post"><input type="hidden" name="c_no" value='+corList[i].c_no+'><input type="submit" class="btn" value='+corList[i].c_name+'>');
          }
          if(corPageNum==1){
             $("#corPageDown").attr("class","disabled");
          } else if (corPageNum == totalCorPage){
             $("#corPageUp").attr("class","disabled");
          } else {
             $("#corPageDown").removeAttr("class");
             $("#corPageUp").removeAttr("class");
          }
      }
   </script>
...
<!-- 스크롤 애니메이션 -->
 <div class="col-md-1 col-lg-2" > 
 	<br>
    <div  style=" height: auto; width: 250px; position: fixed; padding:30px;">
		...
		border="0" onclick="$('html, body').stop().animate( { scrollTop : '700' } );"><br><br>
		<input type="image" class="a" src="../resources/images/testD.png" 
		border="0" onclick="$('html, body').stop().animate( { scrollTop : '2050' } );"><br><br>
		<input type="image" class="a" src="../resources/images/testC.png" 
		border="0" onclick="$('html, body').stop().animate( { scrollTop : '2500' } );">
      </div>
      </div>
<div class="col-xs-12 col-md-10 col-lg-8" >
	<div class="row" style="border: 1px solid #F2F2F2; border-radius: 20px;">
		<div class="col-xs-12 col-md-6 col-lg-6"  >
			...
			<c:if test="${m_name eq null }">
				...
				<br><input type="image" src="../resources/images/t1.png" width="150px;" >
				<h1>${cor_info.c_name }
	...
	<div id="corListDiv">
			...
			<nav>
				<ul class="pager">
					<li id="corPageDown" ><a href="javascript:corPageDown();" >Previous</a></li>
					<li id="corPageUp"><a href="javascript:corPageUp();" >Next</a></li>
				</ul>
			</nav>
			</div>
	...
	<br><br>
	<div class="row" style="border: 1px solid #F2F2F2; border-radius: 20px;">
      <br> 　<input type="image" src="../resources/images/t4.png" width="150px;" >
      <div class="row" >
      ...
      <div role="tabpanel">
         ...
         <div class="tab-content">
           <div role="tabpanel" class="tab-pane active" id="c_2016">
              <canvas id="R2016"></canvas>     	
	              <div class="table-responsive" style="margin-right:180px;">
	              	<table class="table table-hover">
	              		<br><br><thead><h4>　 【 2016년 재무비율 】</h4></thead>
	              		<tbody align="center">
		              		<tr>
		              			<th></th>
		              			<th><!-- 유동비율 --> 
           ...
           <div role="tabpanel" class="tab-pane" id="c_2015">
              <canvas id="R2015"></canvas>
	              <div class="table-responsive" style="margin-right:180px;">
	              	<table class="table table-hover">
	              		<br><br><thead><h4>　 【 2015년 재무비율 】</h4></thead>
		...
		<div class="row" style="border: 1px solid #F2F2F2; border-radius: 20px;">
		<br>
		          　	<input type="image" src="../resources/images/t2.png" width="150px;" >
...
<div class="row" style="border: 1px solid #F2F2F2; border-radius: 20px;">
<br>
          　<input type="image" src="../resources/images/t3.png" width="150px;" >
<div>
		<div class="media">
		  <div class="media-body" id="article" style="padding:50px;">
		  </div>
		</div>
		<nav>
			<ul class="pager">
...
<!-- DaumMap  -->
  ...
  <script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=dcb5411ebb7122004f62f40949d55bbe&libraries=services"></script>
  <script type="text/javascript">
